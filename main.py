...
def login():
  if request.method == 'POST':
    ...
    if user and bcrypt.check_password_hash(user[1], password):
      ...
      session['role'] = aes_decrypt(user_obj['role'])
...
def register():
  if request.method == 'POST':
    ...
    save_user(username, hashed_password, first_name, last_name, "user")
...
def book_slot():
  if request.is_json:
    ...
    insert_booking_2_db(booking_object['date'], booking_object['time'],
                        aes_encrypt(session['username']), booking_object['cancel'])
...
def change_password():
  ...
  if request.method == 'PUT':
    ...
    try:
      ...
      return {"type":"success", "title":"Password Change", "message":"Your Password Has Been Changed."}
    except:
